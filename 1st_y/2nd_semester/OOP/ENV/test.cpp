...
class WeatherCondition; // Forward declaration
...
public:
    Electricity(float eff = 0.0) : efficiency(eff) {}
...
public:
    Thermoel(float eff = 0.0, float c = 0) : Electricity(eff), coal(c) {}
...
public:
    Solar(float eff = 0.0, float a = 0) : Electricity(eff), acreage(a) {}
...
public:
    WeatherCondition(int hour = 0, float speed = 0.0, bool cloud = false, float consume = 0.0) :
        hour(hour), speedWind(speed), Cloudy(cloud), Consume(consume) {}

    void Input() { cin >> hour >> speedWind >> Cloudy >> Consume; }

    float getSpeed() const { return speedWind; }
    bool getCloudy() const { return Cloudy; }
    int getHour() const { return hour; }
    float getConsume() const { return Consume; }
};
...
int main() {
    ...
    // Keep track of each type of electric
    ...
    for (int i = 0; i < hours; i++) {
        ...
        if (temp > 20.0) {
            // Tắt các nguồn điện không phải thuỷ điện và nhiệt điện
